...
- **Quantitative / comparison content** (option tradeoffs, before/after, KPIs, "A is faster than B") gets a chart, not a wall of prose or a bare table — follow `prompts/styles/quantitative-chart.md` for the seven inline-SVG chart primitives and the "color only the winner, mute the rest, badge the conclusion" rules. A comparison that doesn't make the winner obvious has failed. Lint raises a `L3-prefer-chart` warning on analysis/design docs that ship a comparison-signal table with no accompanying chart.
