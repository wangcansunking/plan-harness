...
3. **Three-phase dispatch:**
   ...
   **Phase C — Render HTML**
   ...
   - **Run `lintAndFix()` from `local-proxy/src/html-lint.js` against the saved file.** Shared-asset docs pass `skipRules: ['L1-docgroup', 'L1-active']`. This one call is the auto-fix loop — it runs the lint, applies every deterministic fixer (`.sections` / `.docgroup` / `.sep` wrappers, `class="active"`, locked palette, `max-width` strip, `.crumb` color, shared-asset links, canonical meta re-embed), writes the patched HTML back to disk, and returns `{ fixed: string[], unfixed: error[], wroteBack: boolean }`. The orchestrator does NOT enumerate findings and call `Edit` per rule — the library does that deterministically.

     If `unfixed[]` is empty → lint clean, proceed to validate. If `unfixed[]` has residual findings (those by definition require new content: mockup visuals, sections, etc.), **re-dispatch the Writer once** with the residual findings injected as "previous attempt's lint output", then call `lintAndFix()` again. After two retries (one Writer dispatch + one re-fix pass), if anything remains write `<doc>.lint.json` with the residual findings, surface the failure, and STOP. **Never STOP on the first lint failure** — the auto-fix loop always runs.

     For environments where importing the module fails, `node bin/lint.mjs --fix <path>` is the equivalent shell entry point.
