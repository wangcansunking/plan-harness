...
function getSidebarHTML(title, sections) {
  let links = "";
...
function wrapPage(content, options = {}) {
  const {
    title = "Plan Document",
    subtitle = "",
    meta = "",
...
function generateDashboard(scenarios, options = {}) {
  ...
  function boot(){
    ...
    function renderCommentList(panel, items, orphans){
      ...
      if (orphans && orphans.length > 0) {
        ...
        orphans.forEach(function(o){
          ...
          meta2.textContent = (o.author || '?') + ' \xB7 ' + (o.anchor.migratedFrom ? 'was ' + o.anchor.migratedFrom.slice(0,16) : 'no section');
    ...
    function renderThreadCard(c, root, role, me){
      ...
      head.innerHTML =
        ...
        (chip ? chip : '') +
        '<span class="ph-card-time">' + escHtml(relTime(c.createdAt)) + (c.editedAt ? ' \xB7 edited' : '') + '</span>';
    ...
    function(match, inner) {
      const newInner = inner.replace(
        ...
        function(aMatch, attrs, body) {
          ...
          const basename2 = href.split(/[\\/]/).pop().split(/[#?]/)[0];
          if (!existingSiblings.has(basename2)) return aMatch;
          let newHref = href;
          if (scenarioDir) {
            const absPath = scenarioDir.replace(/\\/g, "/") + "/" + basename2;
    ...
    function(block, inner) {
      const newInner = inner.replace(
        ...
        function(aMatch, attrs, body) {
          ...
          const basename2 = href.split(/[\\/]/).pop().split(/[#?]/)[0];
          if (!basename2 || !/\.html?$/i.test(basename2)) return aMatch;
          if (existingSiblings.has(basename2)) {
            let newHref = href;
            if (scenarioDir) {
              const absPath = scenarioDir.replace(/\\/g, "/") + "/" + basename2;
              newHref = buildDocHref(absPath, projectId);
...
function injectPlanMeta(html, meta) {
...
var init_auth = __esm({
  "src/auth.js"() {
    ...
    ATTEMPT_RESET_MS = 60 * 60 * 1e3;
    MAX_FREE_ATTEMPTS = 5;
    BACKOFF_BASE_MS = 2 * 1e3;
    BACKOFF_MAX_MS = 5 * 60 * 1e3;
    MAX_NAME_LEN = 80;
    PASSWORD_ALPHA = "abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ23456789";
    PASSWORD_LEN = 16;
    password = null;
    hmacKey = null;
...
async function deleteComment(workspaceRoot, scenario, doc, id, actor) {
  ...
  await appendEvent(file, { op: "delete", id, at: (/* @__PURE__ */ new Date()).toISOString(), by: actor.name });
}
...
var NAME_RE, NAME_MAX, ID_RE, BODY_MIN, BODY_MAX, EXACT_MAX, AFFIX_MAX, EDIT_WINDOW_MS, PERSONA_NAMES, PERSONA_SET, MENTION_RE, CommentError, BUCKET_CAPACITY, BUCKET_WINDOW_MS, buckets, sseClients;
...
import { appendFile as appendFile2, mkdir as mkdir2, readFile as readFile2, writeFile } from "node:fs/promises";
import { join as join2, resolve as resolve2, sep as sep2 } from "node:path";
async function readConfig(workspaceRoot, scenario) {
  const file = resolve2(workspaceRoot, "plans", scenario, ".comment-config.json");
  try {
...
function proposalPath(workspaceRoot, scenario, doc, commentId) {
  ...
  const file = resolve2(plansRoot, scenario, ".comments", `${doc}.proposals`, `${commentId}.diff`);
  if (!file.startsWith(plansRoot + sep2)) {
    throw new CommentError("BAD_REQUEST", "path traversal rejected", 400);
  }
  return file;
}
function commentFilePath2(workspaceRoot, scenario, doc) {
  ...
  assertName2(doc, "doc");
  const plansRoot = resolve2(workspaceRoot, "plans");
  const file = resolve2(plansRoot, scenario, ".comments", `${doc}.jsonl`);
  if (!file.startsWith(plansRoot + sep2)) {
    throw new CommentError("BAD_REQUEST", "path traversal rejected", 400);
  }
  return file;
...
function docHtmlPath(workspaceRoot, scenario, doc) {
  ...
  assertName2(doc, "doc");
  const plansRoot = resolve2(workspaceRoot, "plans");
  const file = resolve2(plansRoot, scenario, `${doc}.html`);
  if (!file.startsWith(plansRoot + sep2)) {
    throw new CommentError("BAD_REQUEST", "path traversal rejected", 400);
  }
  return file;
}
async function appendEvent2(file, event) {
...
async function dispatchRevise(workspaceRoot, scenario, doc, commentId, actor) {
  requireHost(actor);
  const file = commentFilePath2(workspaceRoot, scenario, doc);
  await appendEvent2(file, {
    op: "revise",
    id: commentId,
...
async function acceptProposal(workspaceRoot, scenario, doc, commentId, anchor, actor) {
  ...
  const diff = await readFile2(propFile, "utf8");
  ...
  const html = await readFile2(htmlPath, "utf8");
...
function lintHtml(html, ctx = {}) {
  ...
  if (!skip.has("L1-nav")) {
    ...
    if (!nav) {
      ...
      const sep4 = nav.querySelector(".sep");
      if (!sep4) {
        errors.push({
          rule: "L1-nav",
          severity: "error",
...
import { join as join3, basename, extname, resolve as resolve3, sep as sep3, dirname } from "node:path";
import { URL, fileURLToPath } from "node:url";
import { createHash as createHash2 } from "node:crypto";
async function startDashboard(workspaceRoot, port2 = 3847) {
  if (server) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    server.on("error", (err) => {
      if (err.code === "EADDRINUSE") {
        server.close();
        server = null;
        startDashboard(workspaceRoot, port2 + 1).then(resolvePromise, rejectPromise);
      } else {
...
function slugForRoot(rootPath) {
  const base = basename(resolve3(rootPath)) || "project";
...
function startDaemon(port2 = DAEMON_DEFAULT_PORT) {
    ...
    server.on("error", (err) => {
      if (err.code === "EADDRINUSE") {
        ...
        rejectPromise(new Error(
          `[plan-harness] port ${port2} is already in use by another process. The daemon uses a fixed port so links stay stable. Free port ${port2} or set a different daemon port.`
        ));
      } else {
...
async function serveAssetFile(req, res, filePath) {
  ...
  try {
    const data = await readFile3(resolved);
    res.writeHead(200, {
      "Content-Type": mime,
...
async function serveHtmlFile(req, res, filePath, ctx = {}) {
  ...
  if (!guardRoot || resolved !== guardRoot && !resolved.startsWith(guardRoot + sep3)) {
    res.writeHead(403, { "Content-Type": "text/plain" });
  ...
  try {
    ...
    const isScenarioDoc = resolved.includes(`${sep3}plan-harness${sep3}`) && !resolved.includes(`${sep3}_shared${sep3}`);
    let metaJson;
    if (isScenarioDoc) {
      try {
        const metaRaw = await readFile3(resolved.replace(/\.html?$/i, ".meta.json"), "utf-8");
    ...
    if (isScenarioDoc) {
      ...
      if (errCount > 0) {
        const items = result.errors.map((e) => `<li><code>${escapeHtml(e.rule)}</code>: ${escapeHtml(e.message)}</li>`).join("");
        lintBanner = `<div id="__html_lint_banner" style="position:fixed;top:0;left:0;right:0;z-index:9999;background:#f85149;color:#fff;font:13px/1.4 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:10px 16px;border-bottom:2px solid #8b1717;"><strong>html-lint:</strong> ${errCount} structural error(s) \u2014 this doc does not satisfy <code>prompts/_html-base.md</code>. <a href="#" onclick="document.getElementById('__html_lint_banner').remove();return false;" style="color:#fff;float:right;font-weight:bold;">\xD7</a><ul style="margin:6px 0 0;padding-left:24px;">${items}</ul></div>`;
      }
    }
    const injected = lintBanner ? withLightbox.replace(/<body\b[^>]*>/i, (m) => `${m}${lintBanner}`) : withLightbox;
    const headers = {
      "Content-Type": "text/html; charset=utf-8",
      "Cache-Control": "no-cache"
    };
...
function parseScenarioFromPath(absPath) {
  const rel = absPath.startsWith(workspaceRootPath) ? absPath.slice(workspaceRootPath.length).replace(/^[\\/]+/, "") : absPath;
...
function slugifyHeading(text) {
  return String(text).toLowerCase().replace(/<[^>]+>/g, "").replace(/&[a-zA-Z#0-9]+;/g, " ").replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-").replace(/^-|-$/g, "") || "section";
}
...
function contentSectionBounds(html) {
  ...
  const sectionMatch = html.slice(mainStart, mainEnd > mainStart ? mainEnd : void 0).match(/<section\b[^>]*>/i);
...
async function scanScenarios() {
  ...
  for (const rootName of ["plan-harness", "plans"]) {
    const rootDir = join3(workspaceRootPath, rootName);
    let entries;
    try {
      await stat(rootDir);
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        const scenario = await scanScenarioDir(entry.name, join3(rootDir, entry.name));
    ...
    const scenarios = subdirScenarios.length > 0 ? subdirScenarios : rootName === "plans" ? groupFlatFilesIntoScenarios(flatFiles, rootDir) : [];
...
async function scanScenarioDir(name, dirPath) {
  ...
  for (const pt of planTypes) {
    ...
    if (!found) {
      files.push({ type: pt.type, path: join3(dirPath, `${pt.type}.html`), exists: false, todos: 0, done: 0 });
    }
  }
  const totalTodos = files.reduce((s, f) => s + (f.todos || 0), 0);
  const totalDone = files.reduce((s, f) => s + (f.done || 0), 0);
...
function serveLoginPage(req, res) {
  ...
  const retryAfter = parsedUrl.searchParams.get("retry");
...
async function handleLogin(req, res) {
  ...
  const { cookieValue } = createSession(submittedName);
  const cookie = [
...
for (const sig of ["SIGTERM", "SIGINT"]) {
  process.on(sig, () => {
    Promise.resolve().then(() => (init_web_server(), web_server_exports)).then(({ stopDaemon: stopDaemon2 }) => stopDaemon2()).finally(() => process.exit(0));
  });
}
